...
# No cookies/auth are used by this API, so allowing all origins is safe and
# avoids the browser blocking requests from the static frontend during local
# dev (frontend and backend run on different ports/hosts there).
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
